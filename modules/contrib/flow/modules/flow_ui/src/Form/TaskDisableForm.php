...
/**
 * Form for setting a configured task to be inactive.
 */
class TaskDisableForm extends TaskForm {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ?FlowInterface $flow = NULL, ?FlowTaskInterface $task = NULL, ?FlowSubjectInterface $subject = NULL, ?int $task_index = NULL) {
    ...
    $form['title'] = [
      '#type' => 'markup',
      '#markup' => '<h2>' . $this->t('You are about to disable the task <em>@task</em> on <em>@subject</em> from the Flow configuration of <em>@content</em>.', [
    ...
    $form['actions']['submit']['#submit'] = ['::disable', '::redirectAfterSave'];
   ...
   * {@inheritdoc}
   */
  public function disable(array &$form, FormStateInterface $form_state): void {
    if (!$this->flow->access('update')) {
      return;
    }
    $flow = $this->flow;
    $tasks_array = $flow->get('tasks');
    $tasks_array[$this->taskIndex]['active'] = FALSE;
    $flow->setTasks($tasks_array);
    $flow->save();
    $this->messenger->addStatus($this->t('The task has been disabled.'));
  }
