...
class DesignsFieldWrapper implements TrustedCallbackInterface {
  ...
  public function processConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form = $design_form->buildForm($form, $form_state);
    $form['#title'] = $this->t('Wrapper Design');
  ...
  /**
   * Submission for ::buildConfigurationForm().
   *
   * @param array $form
   *   The form.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
  ...
  public function buildSummary($plugin, array &$summary) {
    ...
    if (!empty($settings['design'])) {
      if ($this->designManager->hasDefinition($settings['design'])) {
        $design = $this->designManager->getDefinition($settings['design']);
        $summary[] = $this->t('Wrapper Design: @label', ['@label' => $design->getLabel()]);
      }
      else {
        $summary[] = $this->t('Wrapper Design: @label', ['@label' => $this->t('Missing design')]);
      }
    }
    return $summary;
  }
  ...
  public function entityViewAlter(array &$build, EntityInterface $entity) {
    ...
    foreach (Element::children($build) as $field_name) {
      if (empty($build[$field_name]['#third_party_settings']['designs_field']['design'])) {
        continue;
      }
  ...
  public function entityFormAlter(array &$form, FormStateInterface $form_state) {
    ...
    foreach (Element::children($form) as $field_name) {
      ...
      if (!empty($field_display['third_party_settings']['designs_field']['design'])) {
        $form[$field_name] += [
          '#object' => $entity,
          ...
          '#pre_render' => [[static::class, 'preRenderWidget']],
        ];
      }
    }
  }
  ...
  public static function trustedCallbacks() {
    return ['preRenderFormatter', 'preRenderWidget'];
  }
  ...
  public static function preRenderWidget(array $element) {
    $config = $element['#third_party_settings']['designs_field'];
  ...
  /**
   * Render API: Convert field formatter into design.
   *
   * @param array $element
   *   The render element.
   *
