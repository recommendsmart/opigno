...
class DesignsEntityFieldUi {
  ...
  public function displayFormAlter(array &$form, FormStateInterface $form_state) {
    ...
    $config = $entity->getThirdPartySettings('design');
    ...
    if ($design) {
      ...
      foreach ($design->getContents() as $field_id => $content) {
  ...
  public function submitForm(array $form, FormStateInterface $form_state) {
    ...
    foreach (['design', 'settings', 'content', 'regions'] as $key) {
      if (!empty($values[$key])) {
        $entity->setThirdPartySetting('design', $key, $values[$key]);
      }
      else {
        $entity->unsetThirdPartySetting('design', $key);
      }
    }
  }
