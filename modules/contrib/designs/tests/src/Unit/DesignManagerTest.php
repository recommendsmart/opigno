...
class DesignManagerTest extends UnitTestCase {
  ...
  protected function setUp(): void {
    ...
    $module_a = new Extension('vfs://root', 'module', 'modules/module_a/module_a.designs.yml');
    $this->moduleHandler->getModule('module_a')->willReturn($module_a);
    $this->moduleHandler->getModuleDirectories()
      ->willReturn(['module_a' => vfsStream::url('root/modules/module_a')]);
    ...
    $theme_a = new Extension('vfs://root', 'theme', 'themes/theme_a/theme_a.designs.yml');
    $this->themeHandler->getTheme('theme_a')->willReturn($theme_a);
    $this->themeHandler->getThemeDirectories()
      ->willReturn(['theme_a' => vfsStream::url('root/themes/theme_a')]);
    ...
    $root = vfsStream::url('root/modules/module_a');
    $this->fileSystem->scanDirectory("{$root}/designs", "/^.*\.yml$/")
      ->willReturn([
        "{$root}/designs/test/test.designs.yml" => (object) [
          'uri' => "{$root}/designs/test/test.designs.yml",
        ],
      ]);
    $this->fileSystem->scanDirectory(vfsStream::url('root/modules/theme_a') . '/designs', "/^.*\.yml$/")
      ->willReturn([]);

    $namespaces = new \ArrayObject(['Drupal\module_a' => vfsStream::url('root/modules/module_a/src')]);
    $this->designPluginManager = new DesignManager($namespaces, $this->cacheBackend->reveal(), $this->moduleHandler->reveal(), $this->themeHandler->reveal(), $this->designSourceManager->reveal(), $this->fileSystem->reveal());
  }
