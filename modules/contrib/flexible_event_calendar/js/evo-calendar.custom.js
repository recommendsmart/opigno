...
(function(factory) {
    ...
    EvoCalendar.prototype.buildTheBones = function() {
        ...
        if (!_.$elements.calendarEl.html()) {
            ...
            markup += '<div class="calendar-events">'+
                            ...
                            '<div class="event-list" id="accordion"></div>'+
                        '</div>';
    ...
    EvoCalendar.prototype.addEventList = function(event_data) {
        ...
        markup += '<div class="event-info">' + event_data.name + '</div>';
        markup += '</div>';
        eventListEl.append(markup);
                    ...
                    for (var i = 0; i < 9; i++) { // this loop is for is weeks (rows)
                        for (var j = 0; j < _.$label.days.length; j++) { // this loop is for weekdays (cells)
                            ...
                            if (day <= _.monthLength && (i > 0 || j >= _.startingDay)) {
                                ...
                                //alert(_.options.calendarEvents[i].id)
                                if (_.$label.days[j] === _.initials.weekends.sat || _.$label.days[j] === _.initials.weekends.sun) {
                                    dayClass += ' --weekend'; // add '--weekend' to sat sun
                                }
                                
                                //myarr = _.options.calendarEvents[j].date.split("/");
                                var system_date = _.formatDate(_.$label.months[_.$active.month]+' '+day+' '+_.$active.year, _.options.format);
                                for(var x = 0; x < _.options.calendarEvents.length; x++) {
                                  var count = 0;
                                  //customclass = "";
                                  if(system_date == _.options.calendarEvents[x].date) {
                                    customclass = "highlight";
                                    for(var y = 0; y < _.options.calendarEvents.length; y++) {
                                      if(_.options.calendarEvents[x].id == _.options.calendarEvents[y].id) {
                                        count = count + 1;
                                      }
                                    }
                                    if(count > 1) {
                                      if(_.options.calendarEvents[x].points == "start") {
                                        customclass = "start-highlight"; 
                                      }
                                      else if(_.options.calendarEvents[x].points == "mid") {
                                        customclass = "mid-highlight"; 
                                      }
                                      else if(_.options.calendarEvents[x].points == "end") {
                                        customclass = "end-highlight"; 
                                      }                                      
                                      break;
                                    }                                    
                                  }                                  
                                ...
                                markup += '<td class="'+dayClass+ ' ' + customclass + '">';
