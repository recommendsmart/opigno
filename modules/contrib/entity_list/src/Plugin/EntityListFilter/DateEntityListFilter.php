...
class DateEntityListFilter extends EntityListFilterBase implements ContainerFactoryPluginInterface {
  ...
  public function buildFilter(array $parameters, EntityList $entity_list) {
    ...
    if (!$parameters['settings']['title_wrapper_display']) {
  ...
  public function configurationFilter(array $default_value, EntityList $entity_list) {
    ...
    $configuration['title_wrapper_display'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Display title wrapper'),
      '#default_value' => $default_value['title_wrapper_display'] ?? FALSE,
    ];
   ...
   *   This is field value.
   */
  public static function dateRangeFilter(Query $query, array $fieldSettings, $fieldValue) {
    $zone = new \DateTimeZone(date_default_timezone_get());
    $datetime = new \DateTime($fieldValue, $zone);
    $date = DrupalDateTime::createFromDateTime($datetime);

    if (in_array($fieldSettings['field_type'], ['created', 'changed'])) {
      $query->condition($fieldSettings['field'], $date->getTimestamp(), $fieldSettings['operator']);
    }
    else if (!in_array($fieldSettings['field_type'], ['created', 'changed', 'daterange'])) {
      $query->condition($fieldSettings['field'], $date->format(DateTimePlus::FORMAT), $fieldSettings['operator']);
    }
    else {
      $name = $fieldSettings['name'] === self::FIELD_DATE_END ? $fieldSettings['field'] . '.end_value' : $fieldSettings['field'];
      $query->condition($name, $date->format(DateTimePlus::FORMAT), $fieldSettings['operator']);
    }
  }
   ...
   *   This is field value.
   */
  public static function dateFilter(Query $query, array $fieldSettings, $fieldValue) {
    $zone = new \DateTimeZone(date_default_timezone_get());
    $datetime = new \DateTime($fieldValue, $zone);
    $date = DrupalDateTime::createFromDateTime($datetime);
    ...
    if (in_array($fieldSettings['field_type'], ['created', 'changed'])) {
      $query->condition($fieldSettings['field'], $date->getTimestamp(), $operator);
    }
    else if ($fieldSettings['field_type'] === 'daterange' && !empty($fieldSettings['end_operators'])) {
      $query->condition($fieldSettings['field'], $date->format(DateTimePlus::FORMAT), $operator);
      $query->condition($fieldSettings['field'] . '.end_value', $date->format(DateTimePlus::FORMAT), $fieldSettings['end_operators']);
    }
    else {
      $query->condition($fieldSettings['field'], $date->format(DateTimePlus::FORMAT), $operator);
    }
