...
namespace Drupal\group_storage\Controller;
...
/**
 * Returns responses for 'group_storage' GroupContent routes.
 */
class GroupStorageController extends GroupContentController {
  ...
  /**
   * Constructs a new GroupStorageController.
   *
  ...
  public function addPage(GroupInterface $group, $create_mode = FALSE) {
    ...
    $storage_handler = $this->entityTypeManager->getStorage('storage_type');
    foreach ($this->addPageBundles($group, $create_mode) as $plugin_id => $bundle_name) {
      if (! empty($build['#bundles'][$bundle_name])) {
        $plugin = $group->getGroupType()->getContentPlugin($plugin_id);
        $bundle_label = $storage_handler->load($plugin->getEntityBundle())
          ->label();

        $t_args = ['%storage_type' => $bundle_label];
        $description = $create_mode
          ? $this->t('Add new content of type %storage_type to the group.', $t_args)
          : $this->t('Add existing content of type %storage_type to the group.', $t_args);
