  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($definitions as $definition) {
      $form['entity_types'][$definition->id()] = [
        ...
        '#default_value' => isset($entity_types_config[$definition->id()]) ? $entity_types_config[$definition->id()] : 0,
      ];
      ...
      $bundles = \Drupal::service('entity_type.bundle.info')->getBundleInfo($definition->id());
      foreach ($bundles as $bundle_id => $bundle_name) {
        $form['entity_types'][$definition->id() . '_bundles'][$bundle_id] = [
          '#type' => 'checkbox',
          '#title' => $bundle_name['label'],
          '#default_value' => isset($entity_types_config[$definition->id() . '_bundles'][$bundle_id]) ? $entity_types_config[$definition->id() . '_bundles'][$bundle_id] : 0,
        ];
      }
    }
