...
namespace Drupal\config_terms_views\Plugin\views\filter;

use Drupal\Core\Entity\Element\EntityAutocomplete;
use Drupal\Core\Form\FormStateInterface;
use Drupal\config_terms\TermStorageInterface;
use Drupal\config_terms\VocabStorageInterface;
...
/**
 * Filter by config term id.
 *
 * @ingroup views_filter_handlers
 *
 * @ViewsFilter("config_terms_index")
 */
class ConfigTermsIndex extends ManyToOne {
   ...
   *
   * @var \Drupal\config_terms\VocabStorageInterface
   */
  ...
  /**
   * Constructs a ConfigTermIndex object.
   *
   ...
   * @param \Drupal\config_terms\VocabStorageInterface $vocabulary_storage
   *   The vocabulary storage.
   * @param \Drupal\config_terms\TermStorageInterface $term_storage
   *   The term storage.
   * @param \Drupal\core\Entity\EntityRepositoryInterface
   *   The entity repository.
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, VocabStorageInterface $vocabulary_storage, TermStorageInterface $term_storage, EntityRepositoryInterface $entity_repository) {
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    return new static(
      ...
      $plugin_id,
      $plugin_definition,
      $container->get('entity_type.manager')->getStorage('config_terms_vocab'),
    ...
    if ($this->options['limit']) {
      ...
      if (empty($this->definition['vocabulary'])) {
        $form['vid'] = [
          ...
          '#options' => $options,
          '#description' => $this->t('Select which config vocabulary to show terms for in the regular options.'),
  ...
  protected function valueForm(&$form, FormStateInterface $form_state) {
    ...
    if ($this->options['type'] == 'textfield') {
      ...
      if ($this->options['limit']) {
        ...
        $form['value']['#target_type'] = 'config_terms_term';
        $form['value']['#selection_settings']['target_vocabs'] = [$vocabulary->id()];
      ...
      if (!empty($this->options['hierarchy']) && $this->options['limit']) {
        $tree = $this->termStorage->loadTree($vocabulary->id(), 0, NULL);
        $options = [];

        if ($tree) {
          foreach ($tree as $term) {
            $choice = new stdClass();
            $choice->option = [$term->id() => str_repeat('-', $term->getDepth()) . $this->entityRepository->getTranslationFromContext($term)->label()];
            $options[] = $choice;
          }
        }
      }
        ...
        $options = [];
        if ($this->options['limit']) {
          $terms = $this->termStorage->loadTree($vocabulary->id(), 0, NULL);
      ...
      $form['value'] = [
        '#type' => 'select',
        '#title' => $this->options['limit'] ? $this->t('Select config terms from vocabulary @voc', ['@voc' => $vocabulary->label()]) : $this->t('Select terms'),
        '#multiple' => TRUE,
        '#options' => $options,
