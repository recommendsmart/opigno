...
namespace Drupal\entity_taxonomy\Plugin\migrate;
...
class D7TaxonomyTermDeriver extends DeriverBase implements ContainerDeriverInterface {
  ...
  public function getDerivativeDefinitions($base_plugin_definition) {
    ...
    $vocabulary_source_plugin = static::getSourcePlugin('d7_entity_taxonomy_vocabulary');
    try {
      $vocabulary_source_plugin->checkRequirements();
    }
    catch (RequirementsException $e) {
      // If the d7_entity_taxonomy_vocabulary requirements failed, that means we do not
    ...
    try {
      foreach ($vocabulary_source_plugin as $row) {
        ...
        $this->fieldDiscovery->addBundleFieldProcesses($migration, 'entity_taxonomy_term', $bundle);
