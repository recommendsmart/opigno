...
/**
 * Gets i18n entity_taxonomy terms from source database.
 *
 * @MigrateSource(
 *   id = "d7_entity_taxonomy_term_translation",
 *   source_module = "i18n_entity_taxonomy"
 * )
 */
class TermTranslation extends Term {
  ...
  public function query() {
    $query = parent::query();
    if ($this->database->schema()->fieldExists('entity_taxonomy_term_data', 'language')) {
      $query->addField('td', 'language', 'td_language');
    }
    if ($this->database->schema()->fieldExists('entity_taxonomy_vocabulary', 'i18n_mode')) {
