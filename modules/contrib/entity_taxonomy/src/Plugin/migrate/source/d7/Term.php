...
namespace Drupal\entity_taxonomy\Plugin\migrate\source\d7;

use Drupal\migrate\Row;
use Drupal\migrate_drupal\Plugin\migrate\source\d7\FieldableEntity;

/**
 * entity_taxonomy term source from database.
 *
...
class Term extends FieldableEntity {
  ...
  public function query() {
    $query = $this->select('entity_taxonomy_term_data', 'td')
      ->fields('td')
      ->distinct()
      ->orderBy('tid');
    $query->leftJoin('entity_taxonomy_vocabulary', 'tv', 'td.vid = tv.vid');
    $query->addField('tv', 'machine_name');

    if ($this->getDatabase()
      ->schema()
      ->fieldExists('entity_taxonomy_vocabulary', 'i18n_mode')) {
  ...
  public function prepareRow(Row $row) {
    ...
    // migration.
    $translatable_vocabularies = array_keys(array_filter($this->variableGet('entity_translation_entity_taxonomy', [])));
    $entity_translatable = $this->isEntityTranslatable('entity_taxonomy_term') && in_array($vocabulary, $translatable_vocabularies, TRUE);

    if ($entity_translatable) {
      $source_language = $this->getEntityTranslationSourceLanguage('entity_taxonomy_term', $tid);
    ...
    foreach ($this->getFields('entity_taxonomy_term', $vocabulary) as $field_name => $field) {
      // Ensure we're using the right language if the entity and the field are
      // translatable.
      $field_language = $entity_translatable && $field['translatable'] ? $language : NULL;
      $row->setSourceProperty($field_name, $this->getFieldValues('entity_taxonomy_term', $field_name, $tid, NULL, $field_language));
    }

    // Find parents for this row.
    $parents = $this->select('entity_taxonomy_term_hierarchy', 'th')
