...
namespace Drupal\entity_taxonomy\Plugin\views\argument_default;
...
/**
 * entity_taxonomy tid default argument.
 *
 * @ViewsArgumentDefault(
 *   id = "entity_taxonomy_tid",
 *   title = @Translation("entity_taxonomy term ID from URL")
 * )
 */
class Tid extends ArgumentDefaultPluginBase implements CacheableDependencyInterface {
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    $form['node'] = [
      '#type' => 'checkbox',
      '#title' => $this->t("Load default filter from node page, that's good for related entity_taxonomy blocks"),
    ...
    $form['limit'] = [
      ...
      '#states' => [
        'visible' => [
          ':input[name="options[argument_default][entity_taxonomy_tid][node]"]' => ['checked' => TRUE],
        ],
      ],
    ];
    ...
    $form['vids'] = [
      ...
      '#states' => [
        'visible' => [
          ':input[name="options[argument_default][entity_taxonomy_tid][limit]"]' => ['checked' => TRUE],
          ':input[name="options[argument_default][entity_taxonomy_tid][node]"]' => ['checked' => TRUE],
        ],
      ],
    ];
  ...
  public function getArgument() {
    // Load default argument from entity_taxonomy page.
    if (!empty($this->options['term_page'])) {
      if (($entity_taxonomy_term = $this->routeMatch->getParameter('entity_taxonomy_term')) && $entity_taxonomy_term instanceof TermInterface) {
        return $entity_taxonomy_term->id();
      }
    }
    ...
    if (!empty($this->options['node'])) {
      ...
      if (($node = $this->routeMatch->getParameter('node')) && $node instanceof NodeInterface) {
        ...
        foreach ($node->getFieldDefinitions() as $field) {
          if ($field->getType() == 'entity_reference' && $field->getSetting('target_type') == 'entity_taxonomy_term') {
            $entity_taxonomy_terms = $node->{$field->getName()}->referencedEntities();
            /** @var \Drupal\entity_taxonomy\TermInterface $entity_taxonomy_term */
            foreach ($entity_taxonomy_terms as $entity_taxonomy_term) {
              $entity_taxonomy[$entity_taxonomy_term->id()] = $entity_taxonomy_term->bundle();
            }
          }
        }
        if (!empty($this->options['limit'])) {
          ...
          foreach ($entity_taxonomy as $tid => $vocab) {
        ...
        // Return all tids.
          ...
          return implode($this->options['anyall'], array_keys($entity_taxonomy));
        }
      }
    }
  }
