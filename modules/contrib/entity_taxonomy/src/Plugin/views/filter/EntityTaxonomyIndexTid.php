...
use Drupal\entity_taxonomy\TermStorageInterface;
 ...
 *
 * @ViewsFilter("entity_taxonomy_index_tid")
 */
class EntityTaxonomyIndexTid extends ManyToOne {
   ...
   *   The vocabulary storage.
   * @param \Drupal\entity_taxonomy\TermStorageInterface $term_storage
   *   The term storage.
  ...
  protected function valueForm(&$form, FormStateInterface $form_state) {
    ...
    if ($this->options['type'] == 'textfield') {
      ...
      if ($this->options['limit']) {
        ...
        $form['value']['#target_type'] = 'entity_taxonomy_term';
      ...
      if (!empty($this->options['hierarchy']) && $this->options['limit']) {
        ...
        if ($tree) {
          foreach ($tree as $term) {
            if (!$term->isPublished() && !$this->currentUser->hasPermission('administer entity_taxonomy')) {
              continue;
            }
            $choice = new \stdClass();
