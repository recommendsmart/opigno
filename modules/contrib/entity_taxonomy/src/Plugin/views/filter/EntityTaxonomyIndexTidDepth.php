...
namespace Drupal\entity_taxonomy\Plugin\views\filter;

use Drupal\Core\Database\Database;
use Drupal\Core\Form\FormStateInterface;

/**
 * Filter handler for entity_taxonomy terms with depth.
 *
 ...
 *
 * @ViewsFilter("entity_taxonomy_index_tid_depth")
 */
class EntityTaxonomyIndexTidDepth extends EntityTaxonomyIndexTid {
  ...
  public function query() {
    ...
    $subquery = Database::getConnection()->select('entity_taxonomy_index', 'tn');
    $subquery->addField('tn', 'entity_id');
    $where = ($this->view->query->getConnection()->condition('OR'))->condition('tn.tid', $this->value, $operator);
    $last = "tn";

    if ($this->options['depth'] > 0) {
      $subquery->leftJoin('entity_taxonomy_term__parent', 'th', "th.entity_id = tn.tid");
      $last = "th";
      foreach (range(1, abs($this->options['depth'])) as $count) {
        $subquery->leftJoin('entity_taxonomy_term__parent', "th$count", "$last.parent_target_id = th$count.entity_id");
    ...
    elseif ($this->options['depth'] < 0) {
      foreach (range(1, abs($this->options['depth'])) as $count) {
        ...
        $subquery->leftJoin('entity_taxonomy_term__parent', "th$count", "$last.$field = th$count.parent_target_id");
        $where->condition("th$count.entity_id", $this->value, $operator);
        $last = "th$count";
      }
    }
