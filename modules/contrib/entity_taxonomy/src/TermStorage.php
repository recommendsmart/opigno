...
/**
 * Defines a Controller class for entity_taxonomy terms.
 */
class TermStorage extends SqlContentEntityStorage implements TermStorageInterface {
   ...
   * ID.
   *
   * @var \Drupal\entity_taxonomy\TermInterface[][]
   */
  protected $ancestors;
  ...
  public function resetCache(array $ids = NULL) {
    drupal_static_reset('entity_taxonomy_term_count_nodes');
    $this->ancestors = [];
    $this->treeChildren = [];
    $this->treeParents = [];
    $this->treeTerms = [];
   ...
   *
   * @return \Drupal\entity_taxonomy\TermInterface[]
   *   The parent entity_taxonomy term entities keyed by term ID. If this term has a
  ...
  protected function getParents(TermInterface $term) {
    ...
    if ($ids) {
      $query = \Drupal::entityQuery('entity_taxonomy_term')
   ...
   *
   * @return \Drupal\entity_taxonomy\TermInterface[]
   *   A list of ancestor entity_taxonomy term entities keyed by term ID.
   *
   * @internal
   * @todo Refactor away when TreeInterface is introduced.
   */
   ...
   *
   * @return \Drupal\entity_taxonomy\TermInterface[]
   *   A list of children entity_taxonomy term entities keyed by term ID.
   *
   * @internal
   * @todo Refactor away when TreeInterface is introduced.
   */
  ...
  public function loadTree($vid, $parent = 0, $max_depth = NULL, $load_entities = FALSE) {
    ...
    if (!isset($this->trees[$cache_key])) {
      ...
      if (!isset($this->treeChildren[$vid])) {
        ...
        $query->join('entity_taxonomy_term__parent', 'p', 't.tid = p.entity_id');
        $query->addExpression('parent_target_id', 'parent');
        $result = $query
          ->addTag('entity_taxonomy_term_access')
          ->fields('t')
          ->condition('t.vid', $vid)
