...
/**
 * Defines a class to build a listing of entity_taxonomy vocabulary entities.
 *
...
class VocabularyListBuilder extends DraggableListBuilder {
  ...
  public function getFormId() {
    return 'entity_taxonomy_overview_vocabularies';
  }
  ...
  public function getDefaultOperations(EntityInterface $entity) {
    ...
    if ($entity->access('access entity_taxonomy overview')) {
      $operations['list'] = [
        'title' => t('List terms'),
        'weight' => 0,
    ...
    $entity_taxonomy_term_access_control_handler = $this->entityTypeManager->getAccessControlHandler('entity_taxonomy_term');
    if ($entity_taxonomy_term_access_control_handler->createAccess($entity->id())) {
      $operations['add'] = [
        'title' => t('Add terms'),
        'weight' => 10,
        'url' => Url::fromRoute('entity.entity_taxonomy_term.add_form', ['entity_taxonomy_vocabulary' => $entity->id()]),
      ];
    }
  ...
  public function render() {
    ...
    if (isset($build['table'])) {
      $access_control_handler = $this->entityTypeManager->getAccessControlHandler('entity_taxonomy_vocabulary');
      ...
      if ($create_access->isAllowed()) {
        $build['table']['#empty'] = t('No vocabularies available. <a href=":link">Add vocabulary</a>.', [
          ':link' => Url::fromRoute('entity.entity_taxonomy_vocabulary.add_form')->toString(),
        ]);
      }
      else {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['vocabularies']['#attributes'] = ['id' => 'entity_taxonomy'];
