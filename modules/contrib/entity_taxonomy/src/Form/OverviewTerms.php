...
namespace Drupal\entity_taxonomy\Form;
...
class OverviewTerms extends FormBase {
   ...
   *
   * @var \Drupal\entity_taxonomy\TermStorageInterface
   */
  ...
  public function __construct(ModuleHandlerInterface $module_handler, EntityTypeManagerInterface $entity_type_manager, RendererInterface $renderer, EntityRepositoryInterface $entity_repository, PagerManagerInterface $pager_manager) {
    ...
    $this->storageController = $entity_type_manager->getStorage('entity_taxonomy_term');
    $this->termListBuilder = $entity_type_manager->getListBuilder('entity_taxonomy_term');
  ...
  public function getFormId() {
    return 'entity_taxonomy_overview_terms';
  }
   ...
   *   The form structure.
   */
  public function buildForm(array $form, FormStateInterface $form_state, VocabularyInterface $entity_taxonomy_vocabulary = NULL) {
    $form_state->set(['entity_taxonomy', 'vocabulary'], $entity_taxonomy_vocabulary);
    $vocabulary_hierarchy = $this->storageController->getVocabularyHierarchyType($entity_taxonomy_vocabulary->id());
    $parent_fields = FALSE;

    $page = $this->getRequest()->query->get('page') ?: 0;
    // Number of terms per page.
    $page_increment = $this->config('entity_taxonomy.settings')->get('terms_per_page_admin');
    ...
    $delta = 0;
    $term_deltas = [];
    $tree = $this->storageController->loadTree($entity_taxonomy_vocabulary->id(), 0, NULL, TRUE);
    $tree_index = 0;
    do {
    ...
    $args = [
      '%capital_name' => Unicode::ucfirst($entity_taxonomy_vocabulary->label()),
      '%name' => $entity_taxonomy_vocabulary->label(),
    ];
    if ($this->currentUser()->hasPermission('administer entity_taxonomy') || $this->currentUser()->hasPermission('edit terms in ' . $entity_taxonomy_vocabulary->id())) {
    ...
    $access_control_handler = $this->entityTypeManager->getAccessControlHandler('entity_taxonomy_term');
    $create_access = $access_control_handler->createAccess($entity_taxonomy_vocabulary->id(), NULL, [], TRUE);
    if ($create_access->isAllowed()) {
      $empty = $this->t('No terms available. <a href=":link">Add term</a>.', [':link' => Url::fromRoute('entity.entity_taxonomy_term.add_form', ['entity_taxonomy_vocabulary' => $entity_taxonomy_vocabulary->id()])->toString()]);
    }
    else {
    ...
    foreach ($current_page as $key => $term) {
      ...
      if (in_array($term->id(), $pending_term_ids)) {
        ...
        $form['terms'][$key]['#attributes']['class'][] = 'entity_taxonomy-term--pending-revision';
      }
      ...
      if ($row_position < $back_step || $row_position >= $page_entries - $forward_step) {
        $form['terms'][$key]['#attributes']['class'][] = 'entity_taxonomy-term-preview';
      }

      if ($row_position !== 0 && $row_position !== count($tree) - 1) {
        if ($row_position == $back_step - 1 || $row_position == $page_entries - $forward_step - 1) {
          $form['terms'][$key]['#attributes']['class'][] = 'entity_taxonomy-term-divider-top';
        }
        elseif ($row_position == $back_step || $row_position == $page_entries - $forward_step) {
          $form['terms'][$key]['#attributes']['class'][] = 'entity_taxonomy-term-divider-bottom';
        }
      }
    ...
    if ($update_tree_access->isAllowed()) {
      if ($parent_fields) {
        ...
        $form['terms']['#attached']['library'][] = 'entity_taxonomy/drupal.entity_taxonomy';
        $form['terms']['#attached']['drupalSettings']['entity_taxonomy'] = [
          'backStep' => $back_step,
   ...
   *
   * Because there might be hundreds or thousands of entity_taxonomy terms that need to
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $vocabulary = $form_state->get(['entity_taxonomy', 'vocabulary']);
    $changed_terms = [];
  ...
  public function submitReset(array &$form, FormStateInterface $form_state) {
    /** @var $vocabulary \Drupal\entity_taxonomy\VocabularyInterface */
    $vocabulary = $form_state->get(['entity_taxonomy', 'vocabulary']);
