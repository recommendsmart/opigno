...
/**
 * Base for handler for entity_taxonomy term edit forms.
 *
 * @internal
 */
class TermForm extends ContentEntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    $term = $this->entity;
    $vocab_storage = $this->entityTypeManager->getStorage('entity_taxonomy_vocabulary');
    /** @var \Drupal\entity_taxonomy\TermStorageInterface $entity_taxonomy_storage */
    $entity_taxonomy_storage = $this->entityTypeManager->getStorage('entity_taxonomy_term');
    $vocabulary = $vocab_storage->load($term->bundle());

    $parent = array_keys($entity_taxonomy_storage->loadParents($term->id()));
    $form_state->set(['entity_taxonomy', 'parent'], $parent);
    $form_state->set(['entity_taxonomy', 'vocabulary'], $vocabulary);

    $form['relations'] = [
      '#type' => 'details',
      '#title' => $this->t('Relations'),
      '#open' => $entity_taxonomy_storage->getVocabularyHierarchyType($vocabulary->id()) == VocabularyInterface::HIERARCHY_MULTIPLE,
      '#weight' => 10,
    ];

    // \Drupal\entity_taxonomy\TermStorageInterface::loadTree() and
    // \Drupal\entity_taxonomy\TermStorageInterface::loadParents() may contain large
    // numbers of items so we check for entity_taxonomy.settings:override_selector
    // before loading the full vocabulary. Contrib modules can then intercept
    // before hook_form_alter to provide scalable alternatives.
    if (!$this->config('entity_taxonomy.settings')->get('override_selector')) {
      $exclude = [];
      if (!$term->isNew()) {
        $parent = array_keys($entity_taxonomy_storage->loadParents($term->id()));
        $children = $entity_taxonomy_storage->loadTree($vocabulary->id(), $term->id());
      ...
      $tree = $entity_taxonomy_storage->loadTree($vocabulary->id());
      $options = ['<' . $this->t('root') . '>'];
      if (empty($parent)) {
        $parent = [0];
      }
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($result) {
      case SAVED_NEW:
        ...
        $this->logger('entity_taxonomy')->notice('Created new term %term.', ['%term' => $term->getName(), 'link' => $edit_link]);
        break;

      case SAVED_UPDATED:
        $this->messenger()->addStatus($this->t('Updated term %term.', ['%term' => $view_link]));
        $this->logger('entity_taxonomy')->notice('Updated term %term.', ['%term' => $term->getName(), 'link' => $edit_link]);
        break;
    }
